...
var codeBlock = (text, language) => {
  language = language == null ? void 0 : language.toLowerCase();
...
var getTodoValue = ({ block, blocks, pageTitle }) => {
...
var getTitleValue = ({ block, blocks, pageTitle }) => {
  const text = getTextValue({ blocks, block, pageTitle });
...
var getQuoteValue = ({ block, blocks, pageTitle }) => {
  const text = getTextValue({ blocks, block, pageTitle });
  return quote(text);
};
var getEmphasisTextValue = ({ blocks, block, pageTitle }) => {
...
var getLinkValue = ({ blocks, block, pageTitle }) => {
  let text = getTextValue({ blocks, block, pageTitle });
...
// src/client.ts
import { out as out2 } from "@flowusx/flowus-shared";
...
var FlowUsToMarkdown = class {
  constructor(options) {
    ...
    if (this.config) {
      out2.warning("\u6682\u4E0D\u652F\u6301\u6B64\u914D\u7F6E", JSON.stringify(this.config));
    }
  }
  pageToMarkdown(id) {
    return __async(this, null, function* () {
      if (!this.flowusClient) {
        out2.err("flowusClient is not provided");
        process.exit(1);
      }
    ...
    if (firstValue.type === 0 /* Doc */) {
      ...
      firstValue.subNodes.forEach((blockId) => {
        ...
        if (!transform[curType]) {
          out2.debug(`\u6682\u4E0D\u652F\u6301\u7684\u5757\u7C7B\u578B: ${curType}-${block.title}`);
        } else {
