...
import { out as out3, request } from "@elog/shared";
...
var processHtml = (content) => {
  const processValue = unified().use(rehypeParse).use(() => (tree) => {
    findHead(tree);
...
var FormatExt = class {
  ...
  initFormatExt() {
    if (typeof this.formatExt === "boolean") {
      ...
      out2.warning("\u6587\u6863\u5904\u7406\u62D3\u5C55\u70B9\u914D\u7F6E\u9519\u8BEF\uFF0C\u5C06\u4F7F\u7528\u9ED8\u8BA4\u903B\u8F91");
      return processWordWrap;
    }
  }
  /**
...
var YuqueClient = class {
  constructor(config) {
    ...
    if (!this.config.token || !this.config.repo || !this.config.login) {
      out3.err("\u7F3A\u5C11\u53C2\u6570", "\u7F3A\u5C11\u8BED\u96C0\u914D\u7F6E\u4FE1\u606F");
      out3.info("\u8BF7\u67E5\u9605Elog\u914D\u7F6E\u6587\u6863: https://elog.1874.cool/notion/write-platform");
      process.exit(-1);
    }
  ...
  request(api, reqOpts, custom) {
    return __async(this, null, function* () {
      ...
      if (custom) {
        const res2 = yield request(url, opts);
        return res2.data;
      }
  ...
  getDocDetailList(cachedDocs, ids) {
    return __async(this, null, function* () {
      ...
      out3.info("\u5DF2\u4E0B\u8F7D\u6570", String(articleList.length));
      return articleList;
    });
  }
};
...
var YuqueWithToken = class {
  ...
  getDocList() {
    return __async(this, null, function* () {
      ...
      this.pages = pages;
      out4.info("\u6587\u6863\u603B\u6570", String(this.pages.length));
...
// src/pwd/client.ts
import asyncPool2 from "tiny-async-pool";
import { out as out5, request as request2 } from "@elog/shared";
...
var YuqueClient2 = class {
  ...
  request(api, reqOpts, custom) {
    return __async(this, null, function* () {
      ...
      if (custom) {
        const res2 = yield request2(url, opts);
        return res2.data;
      }
      const res = yield request2(url, opts);
      if (res.status !== 200) {
        out5.err(res);
      }
      return res.data.data;
  ...
  getToc() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        dom.window.close();
        if (!book) {
          out5.warning("\u722C\u53D6\u8BED\u96C0\u76EE\u5F55\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5");
          process.exit(-1);
        }
        this.bookId = book.id;
        return (book == null ? void 0 : book.toc) || [];
      } catch (e) {
        out5.warning("\u722C\u53D6\u8BED\u96C0\u76EE\u5F55\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5", e.message);
        process.exit(-1);
      }
    });
  }
    ...
    return __async(this, null, function* () {
      ...
      if (ids.length) {
        docs = docs.filter((doc) => {
          const exist = ids.indexOf(doc.slug) > -1;
          if (!exist) {
            out5.info("\u8DF3\u8FC7\u4E0B\u8F7D", doc.title);
          }
          return exist;
        });
      }
      if (!(docs == null ? void 0 : docs.length)) {
        out5.access("\u8DF3\u8FC7", "\u6CA1\u6709\u9700\u8981\u4E0B\u8F7D\u7684\u6587\u7AE0");
        return articleList;
      }
      out5.info("\u5F85\u4E0B\u8F7D\u6570", String(docs.length));
      out5.access("\u5F00\u59CB\u4E0B\u8F7D\u6587\u6863...");
      docs = docs.map((item, index) => __spreadProps(__spreadValues({}, item), { _index: index + 1 }));
      const promise = (doc) => __async(this, null, function* () {
        out5.info(`\u4E0B\u8F7D\u6587\u6863 ${doc._index}/${docs.length}   `, doc.title);
        let article = yield this.getDocDetail(doc.slug);
        if (!doc.format && IllegalityDocFormat.some((item) => item === article.format)) {
          out5.warning("\u6CE8\u610F", `\u3010${article.title}\u3011\u4E3A\u4E0D\u652F\u6301\u7684\u6587\u6863\u683C\u5F0F`);
        }
      ...
      yield asyncPool2(this.config.limit || 3, docs, promise);
      out5.info("\u5DF2\u4E0B\u8F7D\u6570", String(articleList.length));
      return articleList;
    });
  }
};
...
var YuqueWithPwd = class {
  ...
  getDocList() {
    return __async(this, null, function* () {
      out6.info("\u6B63\u5728\u83B7\u53D6\u6587\u6863\u5217\u8868\uFF0C\u8BF7\u7A0D\u7B49...");
      let pages = yield this.ctx.getDocList();
      pages = pages.filter((page) => {
        if (!page.format)
          return true;
        if (IllegalityDocFormat.some((item) => item === page.format)) {
          out6.warning("\u6CE8\u610F", `\u3010${page.title}\u3011\u4E3A\u4E0D\u652F\u6301\u7684\u6587\u6863\u683C\u5F0F`);
          return false;
        }
        return true;
      }).filter((page) => {
      ...
      this.pages = pages;
      out6.info("\u6587\u6863\u603B\u6570", String(this.pages.length));
      return pages.map((page) => {
        const timestamp = new Date(page.updated_at).getTime();
        return {
          // 暂时只需要返回这些属性
...
export {
  core_default2 as YuqueWithPwd,
  core_default as YuqueWithToken
};
